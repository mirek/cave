...
- AST module analysis: https://github.com/mirek/ast/pull/30 at `c0059c7`.
  All twenty-one review findings have fixes, public regressions, replies and
...
- Workspace workflow: https://github.com/mirek/cave/pull/249, stacked on
  ...
  Both CI runtime refs and the example README now target `c0059c7`.
...
- Cave self-ingestion at `cfb24b6` produces 3,957 unchanged records: zero mapped,
  ...
  were verified. The latest upstream change only preserves each destructured export
  binding's declaration kind and exact range in syntax-only mode.
...
sub-checkouts. Latest logs: `ast-binding-origin-check.log`,
`ast-binding-origin-build.log`, `workspace-local-binding-runtime.log`, and
