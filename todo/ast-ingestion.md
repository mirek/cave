...
- AST module analysis: https://github.com/mirek/ast/pull/30 at `77cc1d8`.
  All twenty review findings have fixes, public regressions, replies and
...
- Workspace workflow: https://github.com/mirek/cave/pull/249, stacked on
  ...
  Both CI runtime refs and the example README now target `77cc1d8`.
...
- Cave self-ingestion at `cfb24b6` produces 3,957 unchanged records: zero mapped,
  ...
  were verified. The latest upstream change only preserves resolved declaration URIs
  for ambient module imports and re-exports.
...
sub-checkouts. Latest logs: `ast-ambient-uri-check.log`,
`ast-ambient-uri-build.log`, `workspace-local-binding-runtime.log`, and
