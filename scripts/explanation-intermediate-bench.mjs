...
const measurements = []
for (const digits of [64, 512, 4096]) {
  const value = '1' + '0'.repeat(digits - 1)
  for (const operands of [1, 16, 64]) {
    const model = {
      schema: Model.schema,
      variables: [{ id: 'x', sort: 'int', min: value, max: value }],
      constraints: [{ id: 'positive-product', expression: {
        kind: 'gt', left: operands === 1 ? { kind: 'variable', id: 'x' } : {
          kind: 'multiply', operands: Array.from({ length: operands }, () => ({ kind: 'variable', id: 'x' }))
    ...
    measurements.push({ assignmentDigits: digits, operands, modelNumericDigits: 2 * digits + 1,
      exactProductDigits: 1 + (digits - 1) * operands,
      medianMs: [...samplesMs].sort((a, b) => a - b)[2], samplesMs })
  }
}
console.log(JSON.stringify({ format: 'cave.explanation-intermediate-benchmark', version: 1,
  runtime: { node: process.version, platform: process.platform, arch: process.arch },
  method: 'One warmup and five timed Explain.report calls per case, using default limits. Construction and assertions excluded. No backend search. Values are powers of ten; exact product digit counts are derived algebraically. Timings include model validation, digesting, assignment capture and arithmetic evaluation. Maximum product length is 262081 digits; this is not a general resource-limit test.',
  measurements }, null, 2))
