// Default times the outer-only expression; --plans-only inspects the slow both-sides variant without executing it.
...
for (const keys of [10000, 100]) {
  ...
  try {
    ...
    const base = QuerySql.current(QuerySql.claims(QuerySql.asOfBoundary(snapshot)))
    ...
    const variants = {
      ...
      'tx-lookup': `SELECT c.* ${select.replace('c.claim_key = latest.claim_key', process.argv.includes('--plans-only') ? '+c.claim_key = +latest.claim_key' : '+c.claim_key = latest.claim_key')}`
    }
    ...
    const samples = { full: [], 'tx-lookup': [] }
    let expected
    for (let iteration = 0; iteration < 3; iteration++) {
      // Alternate order; assertions and full-row comparisons are untimed.
      for (const mode of iteration % 2 === 0 ? ['full', 'tx-lookup'] : ['tx-lookup', 'full']) {
        const results = []
...
console.log(JSON.stringify({ format: 'cave.current-join-benchmark/v1',
