/** Run alone: reinforcing fraction orders versus a direct cross-product reference. */
...
for (const digits of [1000, 10000, 100000]) {
  ...
  for (const sign of [1n, -1n]) {
    const left = { numerator: String(sign * (n + 1n)), denominator: String(n + 7n) }
    const right = { numerator: String(sign * (n + 4n)), denominator: String(n + 3n) }
    const expected = -Number(sign)
    for (const [mode, compare] of [['reference', reference], ['shortcut', Exact.compare]]) {
      const samples = []
      ...
      console.log(JSON.stringify({ node: process.version, digits, sign: Number(sign), mode,
        warmups: 2, samples: 7, samplesMs: samples, medianMs: [...samples].sort((a, b) => a - b)[3],
