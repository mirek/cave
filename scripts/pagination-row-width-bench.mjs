...
for (const keys of [10000, 100]) {
  ...
  try {
    ...
    const variants = {
      ...
      late: `SELECT materialized.* FROM (SELECT c.id ${select}) selected
        JOIN cave_claim materialized ON materialized.id = selected.id ORDER BY materialized.tx`
    }
    const statements = Object.fromEntries(Object.entries(variants).map(([name, sql]) => [name, store.db.prepare(sql)]))
    const samples = { full: [], late: [] }
    let expected
    for (let iteration = 0; iteration < 3; iteration++) {
      // Alternate order; assertions and full-row comparisons are untimed.
      for (const mode of iteration % 2 === 0 ? ['full', 'late'] : ['late', 'full']) {
        const results = []
...
console.log(JSON.stringify({ format: 'cave.pagination-row-width-benchmark/v1',
