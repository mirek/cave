...
if (process.argv[2] === '--case') {
  ...
  const value = 'v'.repeat(identifierLength)
  const literal = { kind: 'literal', sort: 'enum', domain: id, value }
  const expression = { kind: 'and', operands: Array.from({ length: 1024 }, () => ({ kind: 'eq', left: literal, right: literal })) }
  const model = { schema: Model.schema, variables: [], enums: [{ id, values: [value] }],
    constraints: [{ id: 'first', expression }, { id: 'duplicate', expression }] }
  const stats = Validate.model(model)
  assert.equal(stats.expressionNodes, 6146)
  const keyUnits = expressionKey(expression, new Map()).length
  const outcome = { status: 'satisfied', assignment: {},
  ...
  const paths = ['scripts/solver-enum-key-bench.mjs', 'packages/solver/src/explain.ts',
