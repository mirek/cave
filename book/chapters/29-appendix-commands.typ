...
or only an optional one. The path names a store by content: a SQLite file,
or a CAVE text file replayed into memory for the commands that only read
(Chapter 9); appending commands refuse text files, and a read never creates
a missing database. This table is the map; the chapters are the territory.
