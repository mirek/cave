...
with the problem on standard error, and an unknown command also prints the
usage overview, while an unknown option is an ordinary problem with status
1. Commands that report violations or
