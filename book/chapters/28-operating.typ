...
check` prints its report on standard output, and a rejected `cave add
--check` prints a header and one line per violation on standard error, as
Chapter 13 shows. Interrupting a long-running command
