...
for (const role of ['WHEN', 'VIA', 'BECAUSE'] as const) test(`${role} preserves NOT as a full-claim verb`, () => {
  for (const subject of [Claim.entity('item'), Claim.entity('NOT'), Claim.entity('WHEN'), Claim.text('literal')]) for (const negated of [false, true]) {
    const parent = Claim.of({ subject: Claim.entity('parent'), verb: 'EXISTS', payload: Claim.none })
    const child = Claim.of({ subject, verb: 'NOT', negated,
      payload: Claim.relation(Claim.entity('ready')),
