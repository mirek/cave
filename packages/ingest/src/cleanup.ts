...
/** Retain an in-flight failure if releasing an owned resource also fails. */
export const cleanup = (failures: readonly unknown[], release: () => void): void => {
  try { release() }
  catch (error) {
    if (failures.length === 0) throw error
    const previous = failures[0]
    throw new AggregateError([previous, error], `${message(previous)}; cleanup also failed: ${message(error)}`, { cause: previous })
  }
}
...
export const throwIfCancelled = (signal: AbortSignal | undefined, error: unknown): void => {
  try { signal?.throwIfAborted() }
    ...
    throw new AggregateError([reason, error], `${message(reason)}; ingestion also failed: ${message(error)}`, { cause: reason })
  }
}
