...
| `ast <root>` | `--runtime <path>`, `--name <name>`, `--exclude <glob>`, `--max-records <n>`, `--force`, `--dry-run`, `--json` | Ingest a pnpm workspace through mirek/ast: packages, categorized dependency ranges, source ownership, static imports/re-exports, and exports with JSDoc comments. Successful passes reconcile removals; extraction failures publish nothing. Requires pnpm and a separately built AST runtime. See the [pnpm example](../../examples/pnpm-monorepo/README.md). |
...
### Shell-friendly query output

`cave query --db repos.cave '?repo IS repo' --jsonl | jq -r '.repo'`
prints repository names without comments or binding labels. `--jsonl` emits one
JSON object of variable bindings per match (keys omit `?`, values retain CAVE
query text), for example `{"repo":"foo"}`. Multiple variables share one object;
a fully bound match is `{}` and no matches produce empty stdout. JSON escaping
keeps each record on one physical line. `--json` and `--jsonl` are mutually exclusive.
The usual page limit still applies; a remaining `next: <token>` goes to stderr,
so stdout remains valid JSONL. Pass that token with `--cursor` to continue.
Cursor continuation requires a persistent SQLite store: reopening a text store
replays it and invalidates its snapshot. `--sources` returns its whole overlay
as usual, including for text stores.
