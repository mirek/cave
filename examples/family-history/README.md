# The full tour — family history

The [root README](../../README.md) teaches CAVE step by step on a monorepo
and a market watchlist. This page is the *other* walkthrough: one small set
of family-history notes pushed through every surface the engine has —
transitive and inverse queries, contested beliefs and resolution, sensitivity
ceilings and backups, valid time, MCP, LLM ingestion, rules, actions,
structured connect, extraction evals, alias discovery, reconstruction, sync,
automations, the read surface, cited reports, and the optional solver. Every
output below was captured from an actual run; commands are written from the
repository root with `cave` on the path.

## Notes become claims

Take a note you'd write anyway — [`notes.md`](notes.md):
...
The same knowledge as CAVE — one atomic claim per line ([`notes.cave`](notes.cave)):
...
CAVE runs as a local [MCP server for GitHub Copilot CLI](https://docs.github.com/en/copilot/how-tos/copilot-cli/customize-copilot/add-mcp-servers), giving Copilot tools to record, search, and query your knowledge. The two commands in [Install](../../README.md#install) register one user-level database at `~/cave.db`.
...
ordinary CAVE-Q patterns — [`rules.cave`](rules.cave):
...
From here: `cave mcp --db family.db` serves the store to any MCP client, and `cave help` lists everything. More worked examples — including a production-incident postmortem with confidence-filtered root-cause queries — live in [`examples/`](../).
