...
Append surfaces auto-stamp actor provenance onto claims that carry no `@src:` context — `@src:cli`, `@src:agent/<client>`, `@src:ingest` (§9.5); a written `@src:` wins over the stamp, except lifecycle stamps (connect records, rule conclusions, action effects — §9.5), which are applied alongside it.
